...
class KubeflowDagRunner(tfx_runner.TfxRunner):
  ...
  def run(self, pipeline: tfx_pipeline.Pipeline):
    ...
    with dsl.Pipeline(pipeline.pipeline_args['pipeline_name']) as p:
    ...
    workflow = self._pipeline_injection(p)
    pipeline_meta = PipelineMeta(
        name=pipeline.pipeline_args['pipeline_name'], description='')
